...
namespace MergePdfWebApp.Controllers
{
    public class ValuesController : ApiController
    {
        ...
        public IHttpActionResult Post([FromBody] Details details)
        {
            ...
            //tempParams:GoremYozem=9998&Shana=2022&Numerator=22&MsMahadura=1&StatusHazmana=2&SugTofes=3&SumIsBig=0&ShanaTaktzivit=2022&GoremHeskem=0&ShanaHeskem=0&NuHeskem=0&SapakNo=4508&MsMachsan=607
            ...
            logger.Info($"Post:resDict.Count: {result_count}");
            ParameterValue[] parameters = new ParameterValue[result_count];
            int i = 0;
            ...
            try
            {
                ...
                logger.Info($"rs.LoadReport({reportPath},{historyID})");
                execInfo = rs.LoadReport(reportPath, historyID);

                rs.SetExecutionParameters(parameters, "en-us");
                string SessionId = rs.ExecutionHeaderValue.ExecutionID;
                logger.Info("MergePdfWebApp:Post::SessionID:" + SessionId);
            }
            ...
            try
            {
                ...
                logger.Info($"MergePdfWebApp:Post::rs.Render(format={format},devInfo={devInfo}");
                result2 = rs.Render(format, devInfo, out extension, out mimeType, out encoding, out warnings, out streamIDs);
                logger.Info($"MergePdfWebApp:Post::rs.Render({format},{devInfo},{extension},{encoding},{mimeType},{warnings},{streamIDs}");
                if (null == streamIDs)
                {
            ...
            try
            {
                ...
                logger.Info($"stream.Write(result2, 0, {result2.Length})");
            ...
            try
            {
                ...
                {
                    //sasa test
                    string url = @"http://sql08/ReportServer_DEVOP?/CHOSHEN/Hazmana-Tubin&"
                                + fullRsUrl
                                //+ "GoremYozem=9998&Shana=2022&Numerator=34&MsMahadura=1&StatusHazmana=2&SugTofes=3&SumIsBig=0&ShanaTaktzivit=2022&GoremHeskem=0&ShanaHeskem=0&NuHeskem=0&SapakNo=1005&MsMachsan=607"
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        try
                        {
                            ...
                            client.DownloadFile(address, rsFile);

                        }
                        catch (Exception e)
                        {
                            logger.Error("DownloadFile EXC: " + e.Message);
                        }
                    }
                    ////
